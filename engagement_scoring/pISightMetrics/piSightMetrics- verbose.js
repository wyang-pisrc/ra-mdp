...
function calcOneAspectBayesianProbability(updatedParts, aspectProportion) {
    ...
    for (let label of aspectLabels) {
        ...
        if (PISIGHT_VERBOSE == -4 && label.includes("lead")) {
            console.log(label);
...
function calcOneAspectParts(aspectKYields, aspectProportion, traffic) {
    ...
    for (let label of aspectLabels) {
        ...
        if (PISIGHT_VERBOSE == -1 && label.includes("lead")) {
            console.log(label);
...
function storeVisitedParts(allVisitedConditionalParts) {
    ...
    if (PISIGHT_VERBOSE > 2) {
        for (let [i, k] of Object.entries(allVisitedConditionalParts)) {
            console.log(i, "current allVisitedConditionalParts: ", JSON.stringify(k));
        }
    }
}
...
async function piSightMain(overrideCorePath) {
    ...
    for (let aspectName of PISIGHT_ASPECTS) {
        ...
        if (PISIGHT_VERBOSE == -3 && aspectName.includes("lead")) {
